...
Copied verbatim from ``05_conditioning/splits.py`` so this folder is self-contained
on ``sys.path`` (no import collision with ``05_conditioning``). The split
(seed 42, n_val=1, n_test=2, test = lowest-threshold settings) is identical to
``train_lstm_tfilm_gr.ipynb``.
"""
...
def normalize_setting_params(setting: str) -> list[float]:
    """Map folder-name compressor settings to [0, 1] — matches nablafx data.py."""
