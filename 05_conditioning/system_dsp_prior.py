"""Lightning system for the DSP-prior GR predictor.
...
stateless 3 s crops:
    ...
    loss = Huber(pred_db, gr_db)                       accuracy
         ...
         + prior_weight · Huber(gr_dsp, gr_db)         keeps the DSP path
                                                       calibrated / interpretable
...
class DSPPriorGRSystem(LightningModule):
    def __init__(
        self,
        model: nn.Module,
        lr: float = 1e-3,
    ...
    def _step(self, batch, phase: str):
        ...
        pred, gr_dsp, delta = self.model(dry, params, return_parts=True)
        ...
        prior = masked_huber(gr_dsp, gr_frames, valid, n_valid)
        ...
        loss = huber + self.delta_weight * dloss + self.prior_weight * prior

        with torch.no_grad():
            mae_db = ((pred - gr_frames).abs() * valid).sum() / n_valid
            prior_mae_db = ((gr_dsp - gr_frames).abs() * valid).sum() / n_valid
            delta_mag = (delta.abs() * valid).sum() / n_valid
        ...
        self.log(f"prior_mae_db/{phase}", prior_mae_db, on_step=False, on_epoch=True, batch_size=bs)
        self.log(f"delta_db/{phase}", dloss, on_step=False, on_epoch=True, batch_size=bs)
        self.log(f"delta_mag_db/{phase}", delta_mag, on_step=False, on_epoch=True, batch_size=bs)
        return loss
