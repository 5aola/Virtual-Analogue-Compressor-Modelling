...
Example:
uv run B_analysis/simple-loss-check.py \
...
# Add parent directory to sys.path so root modules (like infer_compressor) can be imported
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))

from B_analysis.infer_compressor import (
    discover_models,
...
from B_analysis.utils import calc_gain_reduction

# ---------------------------------------------------------------------------
# Custom loss helpers
# ---------------------------------------------------------------------------

# Import loss functions from loss.py (same directory)
loss_mod_path = os.path.join(os.path.dirname(__file__), "loss.py")
loss = SourceFileLoader("loss", loss_mod_path).load_module()
